...
def readfile(file_path):
  ...
  if not os.path.isfile(file_path):
      print("{} is not file".format(file_path))
      return
  else:
...
def JaccardSim(ori, target):
    seta = splitWords(ori)[1]
    setb = splitWords(target)[1]    
    denom = len(ori)
    if len(setb) < 10:
        denom = 10
    sa_sb = 1.0 * len(seta & setb) / denom 
    
    return sa_sb

def caculate(name):
    ori = readfile("./data/user/feature/{}".format(name))
    directory = os.listdir("./data/feature")
    reslist = []
    for f in directory:
        target = readfile("./data/feature/" + f)
        res = JaccardSim(ori, target)
        reslist.append({
            "id": f,
            "sim": res,
        })
...
# a = TextSimilarity("../1.txt", "../2.txt")
# b = TextSimilarity("../3.txt", "../2.txt")
# c = TextSimilarity("../1.txt", "../4.txt")
# d = TextSimilarity("../3.txt", "../4.txt")
# print(a.JaccardSim(a.str_a, a.str_b))
# print(b.JaccardSim(b.str_a, b.str_b))
# print(c.JaccardSim(c.str_a, c.str_b))
# print(d.JaccardSim(d.str_a, d.str_b))